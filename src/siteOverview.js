    ...
    const columns = [
        {
            ...
            <div className='align darkTxt tooltiptext'>
                ...
                ${
                    props.getValue().city
                        ? `<p>City: <b>${props.getValue().city}</b></p>`
                        : ''
                }
                ${
                    props.getValue().state
                        ? `<p>State: <b>${props.getValue().state}</b></p>`
                        : ''
                }
                ${
                    props.getValue().country
                        ? `<p>Country: <b>${props.getValue().country}</b></p>`
                        : ''
                }
                ${
                    props.getValue().site_status
                        ? `<p>Status: <b>${
                              props.getValue().site_status
                          }</b></p>`
                        : ''
...
const defineKriObj = function (data, { sparklineData, sparklineWorkflow }) {
    const kriObj = data.all_kris_list
        .filter((kri) => /^kri/.test(kri.kri_id))
        .map((kri) => ({
            ...
            sortingFn: (A, B, columnId) => {
                const flagComparison =
                    Math.abs(A.original[columnId].flag_value) -
                    Math.abs(B.original[columnId].flag_value);
                const scoreComparison =
                    Math.abs(A.original[columnId].selected_snapshot_kri_value) -
                    Math.abs(B.original[columnId].selected_snapshot_kri_value);
